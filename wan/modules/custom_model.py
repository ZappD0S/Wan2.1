...
class CustomWanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, simil_masks, bias_kwargs):
        ...
        simil_masks_float = simil_masks[0].float()
        n_masks = len(simil_masks_float)
        ...
        sums = simil_masks_float.sum(dim=0)
...
class CustomWanI2VCrossAttention(CustomWanSelfAttention):
    ...
    def forward(self, x, context, grid_sizes, simil_masks, bias_kwargs):  # type: ignore
        r"""
        Args:
        ...
        def _compute_attn_mask(
            ...
            [[T, H, W]] = grid_sizes
            L = T * H * W
            # TODO: explain what main_token_mask is
            attn_mask = repeat(
        ...
        def _compute_attn(q, context, context_img):
            b, _, n, d = q.shape
        ...
        if not bias_kwargs["bias"] or bias_method == "none":
            return _compute_attn(q, context, context_img)
        ...
        if bias_method == "regional_prompting":
            ...
            y = F.scaled_dot_product_attention(
                query=rearrange(q, "N L H E -> N H L E"),
                key=rearrange(k, "N S H E -> N H S E"),
                value=rearrange(v, "N S H E -> N H S E"),
                attn_mask=attn_mask,
            )
            # merge heads and channel dims
...
class CustomWanAttentionBlock(nn.Module):
    ...
    def forward(
        self, x, e, seq_lens, grid_sizes, freqs, context, simil_masks, bias_kwargs
    ):
        r"""
        Args:
...
class CustomWanModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, x, t, context, seq_len, bias_kwargs, clip_fea, y):
        ...
        simil_masks_list = []
        active_simil_masks = None

        mask_sharing = bias_kwargs.get("mask_sharing")

        for i, block in enumerate(self.blocks):
            bias_block = bias and blocks_bias_schedule[i]

            bias_kwargs["bias"] = bias_block

            x, last_generated_simil_masks = block(
                x,
                e=e0,
                seq_lens=seq_lens,
                ...
                freqs=self.freqs,
                context=context,
                simil_masks=active_simil_masks,
                bias_kwargs=bias_kwargs,
            )
            assert last_generated_simil_masks is not None

            if mask_sharing == "prev":
